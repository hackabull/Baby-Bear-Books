...
class FlipPage extends Component {
    // sets state of nameOfUser for capturing name
    props;
    constructor(props) {
      super(props);
      this.state = {
    ...
    // captures name input and sends to mongo
    handleSubmit = (event) => {
    ...
    // turns the pages of the book Right and renders dropped images back to ReadyItem
    turnRight = () => {
      this.props.pageTurn();
        if (si >= 1) {
          si--;
    ...
    // turns pages left and renders dropped images back to Ready Item
    turnLeft = () => {
      this.props.pageTurn();
        if (si < right.length) {
          si++;
    ...
    // gets name from mongo to place on pages.
    signUpUser = () => {
